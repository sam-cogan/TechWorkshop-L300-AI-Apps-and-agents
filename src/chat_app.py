    ...
    try:
        while True:
            ...
            try:
                ...
                # Send feedback that agent has been selected
                agent_display_names = {
                    "cora": "Cora (Shopping Assistant)",
                    "cart_manager": "Cart Manager",
                    "interior_designer": "Interior Designer",
                    "interior_designer_create_image": "Interior Designer",
                    "customer_loyalty": "Customer Loyalty"
                }
                await websocket.send_text(fast_json_dumps({
                    "answer": f"Connecting to {agent_display_names.get(agent_name, agent_name)}...",
                    "agent": agent_name,
                    "cart": persistent_cart,
                    "loading": True
                }))
            ...
            try:
                ...
                with tracer.start_as_current_span(f"{agent_name.title()} Agent Call"):
                    ...
                    # Send feedback that agent is working
                    await websocket.send_text(fast_json_dumps({
