...
class AgentFrameworkProductManagementAgent:
    ...
    def __init__(self):
        ...
        self.agent = ChatAgent(
            ...
            instructions=(
                "Your role is to carefully analyze the user's request and delegate to the appropriate specialized agent. "
                'Your primary goal is precise and efficient delegation to ensure customers and employees receive accurate and specialized '
                'assistance promptly.\n\n'
                'DELEGATION RULES - ALWAYS follow these:\n'
                '1. MarketingAgent: Use when the request involves:\n'
                '   - Creating, improving, or rewriting product descriptions\n'
                '   - Marketing strategies or promotional content\n'
                '   - Making descriptions "better", more compelling, or catchier\n'
                '   - Target audience identification\n'
                '   - Any creative content for products\n\n'
                '2. ProductAgent: Use when the request involves:\n'
                '   - Retrieving product information from catalog\n'
                '   - Getting prices, quantities, or availability\n'
                '   - Listing products or product details\n'
                '   - Answering "what products do you have"\n\n'
                '3. RankerAgent: Use when the request involves:\n'
                '   - Ranking or sorting products\n'
                '   - Recommending which product to choose\n'
                '   - Comparing products based on criteria\n\n'
                'IMPORTANT: When asked to improve/make better a description, ALWAYS use MarketingAgent.'
            ),
    ...
    def _get_agent_response(
        ...
        try:
            ...
            logger.warning(f"Failed to parse JSON response: {e}. Using plain text fallback.")
            # Return the plain text message with default status
            return {
                'is_task_complete': False,
                'require_user_input': True,
                'content': str(message),
